...
def download(mes,temps,id,save):
    if current_user: 
            ...
            if  temps == "nouvelle":
                    ...
                    releve=sum(re)
                    if save =="false":
                        res=wkhtmltopdf.render_template_to_pdf('manage/pages/factureFormat.html', download=True, save=False,histo=histo, new_rel=new_rel,Nom=name,image=image,fin=fin,releve=releve)
                        return res
                    if save == "true":
                        res=wkhtmltopdf.render_template_to_pdf('manage/pages/factureFormat.html', download=True,histo=histo, save=True, new_rel=new_rel,Nom=name,image=image,fin=fin,releve=releve)
