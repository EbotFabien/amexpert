...
def gestion(id,save):
    if current_user.TYPE == "Admin":
        ...
        if failed:
            ...
            if save =="false":
                res=wkhtmltopdf.render_template_to_pdf('manage/pages/formatclient2.html',his=his, download=True,total=total, save=False,nro=NRO,factura=factura,Nom=name,image=image)
                return res
            if save == "true":
                res=wkhtmltopdf.render_template_to_pdf('manage/pages/formatclient2.html',his=his, download=True, total=total,save=True,nro=NRO, factura=factura,Nom=name,image=image)
                files=os.listdir(app.config['PDF_DIR_PATH'])
                for fil in files:
                    if fil.endswith('.pdf'):
                        f=fil
